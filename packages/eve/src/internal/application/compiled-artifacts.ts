...
export async function writeDevelopmentCompiledArtifactsFiles(input: {
  ...
  await writeFile(
    bootstrapPath,
    createDevelopmentCompiledArtifactsBootstrapSource({
      agentName: input.compileResult.manifest.config.name,
      host: input.compileResult.manifest.config.host,
    }),
  );
  await writeFile(
    workflowWorldPluginPath,
...
/**
 * The lines every bootstrap needs: the agent-scoped Workflow queue namespace
 * and the authored host, both published to the process before anything else
 * runs. Deep call sites read them without ever seeing an agent definition.
 */
export function createBootstrapProcessInstallLines(input: {
  readonly agentName: string;
  readonly host: HostProviderDefinition | undefined;
}): readonly string[] {
  return [
    ...
    `import { installHostProvider } from ${stringifyEsmImportSpecifier(resolvePackageSourceFilePath("src/internal/host/provider.ts"))};`,
    "",
    `installEveWorkflowQueueNamespace(${JSON.stringify(input.agentName)});`,
    `installHostProvider(${JSON.stringify(input.host)});`,
  ];
}

function createDevelopmentCompiledArtifactsBootstrapSource(input: {
  readonly agentName: string;
  readonly host: HostProviderDefinition | undefined;
}): string {
  return [
    "// Generated by eve. Do not edit by hand.",
    ...createBootstrapProcessInstallLines(input),
    "",
