...
The guided flow first asks how to configure Microsoft Teams. **Use portable
credentials** is the default and works on any host: eve writes the
credential-free channel above and adds `MICROSOFT_APP_ID`,
`MICROSOFT_APP_PASSWORD`, and `MICROSOFT_TENANT_ID` to `.env.example` for you to
fill in against your own Azure Bot registration.

### Vercel Connect

For a managed Teams bot on Vercel, choose **Set up Vercel Connect**. That branch
runs the Vercel Connect CLI, which takes you through managed Microsoft Teams app
creation and installation. It routes verified Activities to `/eve/v1/teams` and
writes:
