...
The flow first asks how to configure GitHub. **Use portable credentials** is the default and works on any host: eve writes a credential-free `agent/channels/github.ts` and adds `GITHUB_APP_ID`, `GITHUB_APP_PRIVATE_KEY`, `GITHUB_WEBHOOK_SECRET`, and `GITHUB_APP_SLUG` to `.env.example` for you to fill in against your own GitHub App, as described in [Bring your own GitHub App](#bring-your-own-github-app).

Choose **Set up Vercel Connect** instead and the flow signs you in to Vercel when needed, creates or links a Vercel project, provisions an app-scoped GitHub Connect client, and registers `/eve/v1/github` as a trigger destination. It then installs `@vercel/connect` and writes `agent/channels/github.ts` with the connector UID.
