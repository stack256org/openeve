...
The Slack channel puts your agent inside a workspace. It handles `@mentions`, DMs, slash commands, shortcuts, and interactive callbacks; replies in threads; shows typing indicators; and turns human-in-the-loop (HITL) prompts into buttons. Portable credentials are the default setup: you supply the bot token and signing secret through environment variables, which works on any host. On Vercel, [Vercel Connect](https://vercel.com/kb/guide/vercel-connect) is available instead: it manages the Slack bot token, verifies inbound requests, supports token rotation and multiple workspace installations, and forwards events to your agent without copying Slack secrets into your project environment. See [Channels](./overview) for the contract this builds on.
...
The command recommends portable credentials, then scaffolds `agent/channels/slack.ts` and installs any required dependency. Choose **Set up Vercel Connect** when the agent runs on Vercel and you would rather Connect held the Slack bot token and signing secret.
...
This is the default. In the setup command, **Use portable credentials** is the environment-variable option; eve adds `SLACK_BOT_TOKEN` and `SLACK_SIGNING_SECRET` to `.env.example`.
