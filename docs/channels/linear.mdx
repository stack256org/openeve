...
The flow first asks how to configure Linear. **Use portable credentials** is the default and works on any host: eve writes a credential-free `agent/channels/linear.ts` and adds `LINEAR_AGENT_ACCESS_TOKEN` and `LINEAR_WEBHOOK_SECRET` to `.env.example` for you to fill in against your own Linear app, as described in [Bring your own Linear app](#bring-your-own-linear-app). Choose **Set up Vercel Connect** for the managed flow below.
