...
The setup first asks how to configure Discord. **Use portable credentials** is the default and works on any host: eve validates the bot token, registers the application command, scaffolds a credential-free channel, and writes `DISCORD_APPLICATION_ID`, `DISCORD_BOT_TOKEN`, and `DISCORD_PUBLIC_KEY` to `.env.local`. You then point the application's Interactions Endpoint URL at your deployed `https://…/eve/v1/discord`.
...
Choose **Set up Vercel Connect** instead and the setup also:
...
- configures the Discord application's Interactions Endpoint URL; and
- scaffolds the channel with Connect-managed credentials.

Either way the setup guides you through creating a Discord application and bot, registers an application command with a required `message` option, and prints the bot installation URL.
...
The Connect-managed channel looks like this:
