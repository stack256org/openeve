...
`eve init` scaffolds an `agent/channels/eve.ts` with a production placeholder so you replace it before going live. The generated channel accepts HTTP Basic when `EVE_API_PASSWORD` is set, falls back to localhost access, and ends with `placeholderAuth()`, which returns a setup-focused 401 in production until you swap it for real auth. It does not register `vercelOidc()`, which never matches off Vercel; add it when Vercel-issued deployment tokens are part of your trust model. Delete the file and eve selects its default channel source with `[vercelOidc(), localDev(), placeholderAuth()]`, which rejects all production traffic.
