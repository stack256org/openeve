...
The eve Software Factory template: Foreman, an orchestrator agent built on the [eve](https://eve.dev) framework that turns work items into reviewed draft pull requests on the configured repository (`FACTORY_REPO`). Work arrives from GitHub (an issue labeled `factory` runs unattended; @Foreman mentions from owners/members/collaborators run attended) and from Linear Agent Sessions. The orchestrator moves every item through four declared subagent stations in order: **classifier** (triage, fast model) → **analyst** (plan + acceptance criteria, own repo checkout) → **implementer** (codes, verifies, pushes a feature branch from its own checkout) → **reviewer** (independent verdict on the pushed branch, different model vendor, max 2 revision cycles). The orchestrator then opens a draft PR; marking it ready parks on human approval, closing or reopening issues runs as reversible triage, and merging is not in the tool surface. Per-user preferences live in the **document store** under `EVE_DATA_DIR`, alongside a shared, per-repo **factory brain** (durable notes about the target repository) under a reserved prefix, readable by every run but writable only by trusted callers, and **handoff artifacts**: long Markdown documents (research memos, analysis detail) that stations pass to each other by id under the reserved `artifacts/` prefix instead of inlining them through the orchestrator. The pipeline lives in `agent/instructions.ts`.
...
- **Connections** are MCP servers: `linear` (`https://mcp.linear.app/mcp`, app-scoped `LINEAR_AGENT_ACCESS_TOKEN` shared through `linearAuth` in `agent/lib/constants.ts`). Its connection-wide `approval` predicate denies writes on unattended runs.
...
- **Evals** live in `evals/` (`defineEval`, one file per case; `evals.config.ts` sets the judge model). Category directories are the failure taxonomy (`routing/`, `safety/`, `pipeline/`); `helpers.ts` carries the shared write-tool list so read-only evals assert deny-by-default (`notCalledTool` over the whole list). Tags: `fast` (cheap loop), `slow`, `needs-credentials` (asserts calls that must succeed against real provider credentials), `pipeline` (pushes a real branch; opt-in).
...
- **Never commit secrets.** `.env*` is gitignored. Every credential is read from env (`GITHUB_APP_*`, `GITHUB_TOKEN`, `LINEAR_AGENT_ACCESS_TOKEN`, `LINEAR_WEBHOOK_SECRET`, `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`), resolved per call and never exposed to the model. `FACTORY_REPO` is required at module load (`requireEnv`), so a missing value fails discovery; `FACTORY_SETUP_COMMAND` and `EVE_DATA_DIR` are optional.
- **Git safety is structural; keep it that way.** Sandbox git always targets the literal `https://github.com/<FACTORY_REPO>.git` URL, never `origin` (remote config in a sandbox is model-writable), with `GITHUB_TOKEN` injected at the sandbox firewall (`brokerPolicy`) so it never enters the sandbox. Everything interpolated into a git command passes `validateBranch`, which also refuses `main`/`master`. Don't weaken these when adding git capabilities.
...
- Every reserved prefix is declared in the namespace registry in `agent/lib/documents.ts`, alongside the shared read/write/delete helpers all document tools go through. That module is also where every key is matched against an anchored pattern before it is joined onto `EVE_DATA_DIR` — an unvalidated key is a path traversal. Any general-purpose document tool added later must consult the registry's guards before acting, so a managed document can't be reached through a generic file operation; add a namespace there, never as a loose constant in a feature module.
- For per-user storage, derive the key from the resolved principal (`ctx.session.auth.current`), never from model input — see `agent/lib/user-preferences.ts`. The preference files live under the reserved `user-preferences/` prefix, reachable only through the principal-scoped preference tools.
- The shared **factory brain** derives its key from `FACTORY_REPO`, never from model input or a caller principal — see `agent/lib/factory-brain.ts`. It lives under the reserved `factory-brain/` prefix, reachable only through `read_factory_brain` / `update_factory_brain`; writes are gated by `factoryBrainPolicy` (unattended runs denied, trusted callers direct, everyone else parks) so an untrusted issue body can't poison the context every future run reads.
- **Handoff artifacts** live under the reserved `artifacts/` prefix, reachable only through `save_artifact` / `read_artifact` — see `agent/lib/artifacts/config.ts`. Ids are model-supplied on read, so every id must pass the anchored `ARTIFACT_ID_PATTERN` (no dots or slashes) before it is interpolated into a document key; that is what keeps a station from addressing the brain or a preference file through the artifact tools. Saves never overwrite and are size-bounded, which keeps both tools inert enough to live in task-mode stations without approval.

## Before committing

- `pnpm validate` passes (Ultracite check, `tsc`, and `eve info` with 0 errors / 0 warnings).
- No secrets, `node_modules`, stored state (`data/`), or build output (`.eve`, `.output`) staged.
