 ...
 *
 * @remarks
 * - Tools appear to the model as `github__<name>`; the extension reads
 *   `GITHUB_TOKEN` from the environment and never exposes it to the model.
 *   `context` fills `owner`/`repo` from `FACTORY_REPO` so tool calls omit
 *   them.
 ...
 *   in the GitHub UI), and so are repo administration, gists (they 403 under
 *   installation tokens), releases, and CI mutation.
