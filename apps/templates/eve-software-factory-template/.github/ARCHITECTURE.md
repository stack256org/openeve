...
This is a software factory built on the [eve](https://eve.dev) agent framework: the root agent is an orchestrator that takes work items from GitHub (an issue labeled `factory`, or an @Foreman mention) and Linear (Agent Sessions), and moves each through four stations, each a declared subagent with its own instructions, sandbox, and tool surface: **classifier** (triage), **analyst** (plan with acceptance criteria, grounded in a repo checkout), **implementer** (executes the plan in its own checkout, runs the repo's checks, pushes a feature branch), and **reviewer** (independent verdict on the pushed branch, different model vendor, up to 2 revision cycles). The finished product is a draft pull request on `FACTORY_REPO`. People stay in the loop where judgment lives: marking a PR ready stops the session to request approval, merging isn't in the tool surface at all, and unattended runs are denied everything except labels, progress comments on their own intake issue, closing or reopening issues, and draft PRs. Closing an issue runs ungated for every caller: it is reversible triage, and a reopen undoes it. The agent runs the same way locally (`eve dev`) and on any host that runs Node 24 (`eve build`).
...
agent/
  ...
  channels/
    github.ts               # eve GitHub channel; credentials from GITHUB_APP_* env; botName from FACTORY_BOT_NAME/GITHUB_APP_SLUG; four hooks: onComment (mention, association-gated, stamps trusted), onIssue ('factory' label -> unattended pipeline under the autonomous principal), onCheckSuite (red CI on factory/* PRs -> unattended fix loop, capped at 2 attempts), onPullRequest (summary comment on opened PRs, bots skipped)
    linear.ts               # eve Linear channel; credentials from LINEAR_AGENT_ACCESS_TOKEN/LINEAR_WEBHOOK_SECRET env; Agent Sessions; stamps trusted (workspace membership is the gate), injects requester name
  ...
  sandbox.ts                # root sandbox (microsandbox VM); the GitHub channel checks the triggering thread's ref out here
  subagents/
  ...
  tools/
    ...
    read_factory_brain.ts     # load the shared factory brain (open to every run)
    update_factory_brain.ts   # write the shared factory brain (factoryBrainPolicy: trusted-write)
    read_artifact.ts          # read a handoff artifact by id (read-only, open to every run)
  lib/
    ...
    documents.ts            # the shared document layer: reserved-namespace registry (all prefixes + guards), anchored key validation, and read/write/delete helpers over EVE_DATA_DIR
    user-preferences.ts     # principal-scoped document key derivation
    factory-brain.ts        # FACTORY_REPO-scoped document key derivation + size bound
    artifacts/              # handoff artifacts: config.ts (id pattern, kinds) + tools.ts (save/read tool factories)
    github/
      credentials.ts        # GITHUB_TOKEN reader shared by the extension and the git helpers
...
| Factory brain     | `agent/tools/{read,update}_factory_brain.ts` + `agent/lib/factory-brain.ts`                                | Tools            | Shared, durable notes about the target repository on disk, keyed to `FACTORY_REPO` (never model input); reads open to every run, writes gated by `factoryBrainPolicy` (trusted-write)                                                |
| Handoff artifacts | `agent/lib/artifacts/` + per-station `tools/{save,read}_artifact.ts` + root `agent/tools/read_artifact.ts` | Tools            | Long Markdown documents stations pass by id: researcher and analyst save, analyst/implementer/reviewer read, the orchestrator relays only the id; ids validated by an anchored pattern so they can't escape the reserved prefix      |
...
- **GitHub** (external): the repository and issue tracker the factory works on. Tool access goes through `@github-tools/eve-extension` with a `GITHUB_TOKEN` read from the environment; git access in station sandboxes authenticates at the sandbox firewall (see Security).
- **Linear** (external): where delegated work arrives and cross-references land. Access via Linear's MCP server with the app-scoped `LINEAR_AGENT_ACCESS_TOKEN` (scopes `read`, `write`, `issues:create`, `comments:create`).
- **Document store** (`EVE_DATA_DIR`, default `./data`): per-user preferences under the reserved `user-preferences/<hashed-principal>.md` prefix, reachable only through the principal-scoped preference tools; the shared factory brain under the reserved `factory-brain/<hashed-repo>.md` prefix, reachable only through the factory-brain tools; and handoff artifacts under the reserved `artifacts/<validated-id>.md` prefix, reachable only through the artifact tools. Every key is matched against an anchored pattern before it is joined onto the data directory. Point `EVE_DATA_DIR` at a mounted volume on a host with an ephemeral filesystem.
- **microsandbox VMs**: the root sandbox holds the channel's thread checkout; each repo-facing station's sandbox holds its own clone of `FACTORY_REPO` (cloned once per template build via `factoryBootstrap`, moved to the current default branch per session via `factoryOnSession`). Local to the host, not durable application stores.
...
| ---------------------- | ------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GitHub                 | Label intake, mentions, and PR events in; comments, branches, and draft PRs out      | eve GitHub channel (`GITHUB_APP_*` env) + `@github-tools/eve-extension` (`GITHUB_TOKEN`); station git via the firewall-brokered `GITHUB_TOKEN`            |
| Linear (channel + MCP) | Agent Sessions in; issue creation, comments, cross-references out                    | eve Linear channel (`LINEAR_AGENT_ACCESS_TOKEN`, `LINEAR_WEBHOOK_SECRET`); MCP connection to `mcp.linear.app` sharing the same token through `linearAuth` |
| Document store         | Per-user preference storage, the shared factory brain, and station handoff artifacts | Files under `EVE_DATA_DIR` (default `./data`), addressed by validated keys                                                                                |
| Model providers        | Model access for the root and every station                                          | Direct provider calls (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`) wired in `agent/lib/models.ts`; the reviewer deliberately runs a different vendor           |
| microsandbox (local)   | Isolated runtimes: root checkout + three station clones                              | `agent/sandbox.ts` and `agent/subagents/*/sandbox.ts` (`microsandbox()` backend, shared builders in `agent/lib/github/repo-sandbox.ts`)                   |

## Deployment & infrastructure

- **Platform:** Node 24 on macOS with Apple Silicon, or on glibc Linux with KVM (`/dev/kvm`, or `MSB_PATH` for a custom runtime). Build with `eve build`. Nothing is hosted: sandboxes are local microsandbox VMs. That backend supports no other host, so Intel macOS, Windows, musl Linux, and serverless platforms are all out; the process starts there and the first turn needing a sandbox fails naming the host. The backend is pinned rather than defaulted because only `vercel()` and `microsandbox()` implement the firewall brokering the git surface depends on (see Security).
- **Webhooks:** point the GitHub App at `/eve/v1/github` (subscribe to `issues`, `issue_comment`, `pull_request_review_comment`, `pull_request`, and `check_suite`) and the Linear webhook at `/eve/v1/linear` (AgentSessionEvent). The GitHub App installation needs write access to contents, issues, and pull requests on `FACTORY_REPO`.
- **Environment:** `GITHUB_APP_ID`, `GITHUB_APP_PRIVATE_KEY`, `GITHUB_APP_SLUG`, `GITHUB_WEBHOOK_SECRET`, `GITHUB_TOKEN`, `LINEAR_AGENT_ACCESS_TOKEN`, `LINEAR_WEBHOOK_SECRET`, `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`; `FACTORY_REPO` (required at module load; a missing value fails discovery); optional `EVE_DATA_DIR` and `FACTORY_SETUP_COMMAND` (runs inside the clone at template build).
- **Local development:** `pnpm dev` runs the same runtime in a TUI, reading `.env`. The webhook surfaces run against a deployment. The dev principal is untrusted by design, so approval cards surface in the TUI.
...
- **Git credentials never enter a sandbox.** Clones, fetches, and pushes target the literal `https://github.com/<FACTORY_REPO>.git` URL, never the model-writable `origin` remote, and the installation token is injected at the sandbox firewall as a header transform on egress to github.com (`brokerPolicy`), dropped again in a `finally`. On microsandbox the header reaches git inside the VM only as an opaque placeholder, substituted for the real credential by the firewall on the way out, so reading the sandbox environment yields nothing usable. This is what pins the backend: `docker()` and `justbash()` cannot express a per-domain header transform, and the workarounds that make them appear to work (an authenticated remote URL, a credential helper, `env`) all move the token inside the sandbox.
