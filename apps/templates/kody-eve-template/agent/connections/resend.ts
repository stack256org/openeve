 ...
 * weekly digest's cron run, which has no signed-in user and so could not resolve
 * a per-user credential (`principal_required`). The key is resolved per call and
 * never exposed to the model.
 */
export default defineMcpClientConnection({
  auth: {
    getToken: () => Promise.resolve({ token: requireEnv("RESEND_API_KEY", "re_123") }),
  },
  description:
