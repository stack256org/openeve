...
agent/
  ...
  channels/
    github.ts               # eve GitHub channel (GitHub App env credentials); botName "Kody", @mentions reply in-thread (gated to owner/member/collaborator commenters), onPullRequest posts a summary comment on opened PRs (bots skipped)
    linear.ts               # eve Linear channel (LINEAR_AGENT_ACCESS_TOKEN + LINEAR_WEBHOOK_SECRET); Agent Sessions, onAgentSession injects requester email; dev-only webhook-trust flag
  ...
  lib/
    constants.ts            # requireEnv + linearAuth: shared app-scoped Linear token from the environment
    assets.ts               # data directory, anchored key validation, content types
    user-preferences.ts     # principal-scoped storage key + reserved-prefix guard (shared helper)
...
| GitHub access       | `agent/tools/github.ts`                                                              | Tool             | GitHub Tools SDK (`createGithubTools`, maintainer preset) on `GITHUB_TOKEN`: read and triage issues on the repo; issue-conversation writes run without approval, higher-impact writes keep the SDK's approval default                                                                                                                                                                                                                               |
...
- **GitHub** (external): the repository and issue tracker the agent digests and triages. All access goes through the GitHub Tools SDK, authenticated with `GITHUB_TOKEN` read from the environment; no token in code.
...
- **The data directory** (`<EVE_DATA_DIR>/assets`, defaulting to `./data/assets`): per-user preferences under the reserved `user-preferences/<hashed-principal>.md` prefix, reachable only through the principal-scoped preference tools. Back up that one directory and the agent's durable state moves with it.
- **The sandbox** (`/workspace/skills/...`): holds the seeded skill files the model reads. Not a durable application data store.
...
| ---------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GitHub                 | Issue/PR mentions and PR-opened events in, in-thread replies and PR summary comments out; issue reads and triage | eve GitHub channel on a GitHub App (`GITHUB_APP_ID`, `GITHUB_APP_PRIVATE_KEY`, `GITHUB_WEBHOOK_SECRET`) + GitHub Tools SDK (maintainer preset) on `GITHUB_TOKEN`                                                                                                       |
| Linear (channel + MCP) | Agent Sessions in; issue creation, comments, and cross-references out                                            | eve Linear channel on `LINEAR_AGENT_ACCESS_TOKEN` + `LINEAR_WEBHOOK_SECRET` (with an `onAgentSession` hook adding the requester's email to context); MCP connection to `mcp.linear.app` sharing the same token through `linearAuth`                                    |
...
| Local filesystem       | Per-user preference storage                                                                                      | `node:fs/promises` under `EVE_DATA_DIR`, via `agent/lib/assets.ts`                                                                                                                                                                                                     |
| Anthropic, OpenAI      | Model access                                                                                                     | `@ai-sdk/anthropic` and `@ai-sdk/openai` called directly, reading `ANTHROPIC_API_KEY` and `OPENAI_API_KEY`; the root model is set in `agent/agent.ts` and the subagent sets its own in `agent/subagents/researcher/agent.ts`                                           |
| Docker                 | Isolated runtime that holds seeded skill files                                                                   | `agent/sandbox.ts` (`docker()` backend)                                                                                                                                                                                                                                |

## Deployment & infrastructure

- **Platform:** any host that runs Node 24 and can reach a Docker daemon. `pnpm build` produces the deployable bundle.
- **Webhooks:** the GitHub App's webhook points at `/eve/v1/github`, the Linear app's at `/eve/v1/linear`, and Resend's inbound webhook at `/eve/v1/resend`.
- **Environment:** GitHub App credentials `GITHUB_APP_ID`, `GITHUB_APP_PRIVATE_KEY`, and `GITHUB_WEBHOOK_SECRET`, plus `GITHUB_TOKEN` for the tools; Linear `LINEAR_AGENT_ACCESS_TOKEN` and `LINEAR_WEBHOOK_SECRET`; model keys `ANTHROPIC_API_KEY` and `OPENAI_API_KEY`; Resend auth `RESEND_API_KEY` and `RESEND_WEBHOOK_SECRET`; sender identity `RESEND_FROM_ADDRESS` and optional `RESEND_FROM_NAME` (compiled into the system prompt at build time; the name falls back to "Kody"); digest config `DIGEST_REPO` and `DIGEST_EMAIL` (required at module load; a missing value fails discovery); `REDIS_URL` for `@chat-adapter/state-redis` (any Redis, hosted or a local container); and `EVE_DATA_DIR` for durable storage (default `./data`, which should be a mounted volume). The email channel reads the same two sender variables, so channel replies and agent-sent mail share one identity.
- **Local development:** `pnpm dev` runs the same runtime in a TUI, reading `.env.local`. The chat surfaces (GitHub, Linear, email) need a public URL, so use a tunnel or a deployment. Schedules never fire on cadence in dev; trigger the digest once with `POST /eve/v1/dev/schedules/weekly-digest`.

## Security considerations

- **Inbound route auth** (`agent/channels/eve.ts`): `localDevUser` plus HTTP Basic, and the Basic credential is only registered when `EVE_API_PASSWORD` is set, so an unset variable leaves the route closed rather than accepting an empty password. Channel traffic is authenticated by each channel's own webhook secret. `localDevUser` defers the trust decision to the framework's `localDev()` and only upgrades the resolved dev principal to a user, so user-scoped features work from the dev TUI without affecting production.
- **Outbound auth:** every outbound credential is a static token read from the environment and resolved per call, never exposed to the model: `GITHUB_TOKEN` for the GitHub tools, `LINEAR_AGENT_ACCESS_TOKEN` shared through `linearAuth` for the Linear connection, and `RESEND_API_KEY` for Resend (a static token works in the cron run, which has no signed-in user). No credentials live in code, and `.env*` is gitignored except `.env.example`.
...
- **Per-user isolation:** the preference tools derive their key from the resolved principal (`ctx.session.auth.current`), never from model input, so a session can only touch its own user's file; the id is hashed so the stored path carries no raw identifier. Preference files live under the reserved `user-preferences/` prefix, and `agent/lib/assets.ts` is the only place a key becomes a path, matching it against an anchored pattern first. The files sit in the data directory unencrypted, so preferences are scoped within the agent, not confidential at rest.
...
- Digest memory: tracking which issues were already reported (e.g. in the data directory or Redis) so "new this week" is computed against the last digest rather than issue timestamps alone.
