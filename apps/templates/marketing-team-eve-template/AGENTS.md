...
This repository holds a team of marketing agents built on the [eve](https://eve.dev) agent framework. The root **lead** agent holds the shared product picture (brand context on disk, under `EVE_DATA_DIR`) and routes each request to one specialist: `product-marketer` for positioning and messaging, `content-marketer` for long-form pieces, `social-media-coordinator` for short-form posts and the Typefully queue, `seo` for organic search work, or `email` for adapting copy into mail and running Resend. The five specialists have no subagents of their own: each does its own web research and its own review pass inline. The lead's workflow lives in `agent/instructions.md`; each specialist's lives in `agent/subagents/<id>/instructions.md`.
...
| Directory               | Holds                                                                                                                                                                              |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lib/assets/`           | the data directory, anchored key validation, reserved-prefix guards, and the five asset tool factories                                                                             |
| `lib/brand-context/`    | the brand context asset key, its size cap, and its two tool factories                                                                                                              |
| `lib/user-preferences/` | the principal-scoped asset key and its size cap                                                                                                                                    |
| `lib/models/`           | every agent's model, as `MODELS`. Each `agent.ts` reads its entry rather than building a model itself                                                                              |
...
The third layer is the identity, and it is the weakest of the three. Resend authenticates with one account-scoped `RESEND_API_KEY`, the same shape as Notion, so Resend records the send against the key rather than against the person who asked for it. The approval prompt is the only thing that names that person, which is why every send gates unconditionally: remove a gate here and a campaign goes out with nobody's name on it. Mint the key with the narrowest permission Resend offers.
...
- Document exported config with **TSDoc** (`@remarks`, `@param`, `@returns`, `@defaultValue`, `@see`). Avoid inline `//` comments; put rationale in the TSDoc block instead. Keep infrastructure plumbing out of it: no ambient credential mechanics, no hosting-provider explanations.
- **`@ai-sdk/anthropic` is pinned exactly, not with a caret.** It depends on an exact `@ai-sdk/provider`, and so does `ai`. A caret range resolves a newer provider than `ai` pins, pnpm installs two copies, and `tsc` then rejects the model passed to `defineAgent` because the two `LanguageModelV4` types are structurally different. The pinned version is the newest that shares `ai`'s provider. Bump it only alongside `ai`, and run `pnpm typecheck` after.
...
- **Never commit secrets.** `.env*` is gitignored except for `.env.example`, which lists every variable with an empty value. Every credential is an environment variable read inside `getToken` and never inlined: `NOTION_API_KEY`, `RESEND_API_KEY`, and `TYPEFULLY_API_KEY` for the connections, `SLACK_BOT_TOKEN` and `SLACK_SIGNING_SECRET` for the channel, `ANTHROPIC_API_KEY` for the models. All three service tokens are account-scoped rather than per-user, so mint each with the narrowest permission that covers the work.
