...
const renderTable = (table: Table): string => {
  const widths = table.columns.map((heading, column) =>
    Math.max(heading.length, ...table.rows.map((row) => (row[column] ?? "").length)),
  );
  const line = (cells: readonly string[]): string =>
    table.columns.map((_, column) => (cells[column] ?? "").padEnd(widths[column])).join(GUTTER);
...
export default defineTool({
  ...
  inputSchema: z.object({
    ...
    postsTable: tableSchema.describe("Post analytics: one row per post, cells as strings."),
    summary: z
      .string()
      .max(1000)
      .optional()
      .describe("Summary of the report, 1-2 sentences. Write this in a casual tone."),
  }),
  outputSchema: z.object({
    channel: z.string().optional().describe("The Slack channel the digest was posted into."),
    error: z.string().optional(),
    posted: z
      .boolean()
      .describe("False when there was no Slack thread to post into, or Slack refused the message."),
    report: z
      .string()
      .optional()
      .describe(
