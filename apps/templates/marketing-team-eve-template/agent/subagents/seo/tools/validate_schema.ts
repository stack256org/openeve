...
const nodesOf = (doc: unknown): JsonLdNode[] => {
  if (Array.isArray(doc)) {
    return doc.filter((n): n is JsonLdNode => typeof n === "object" && n !== null);
  }
  ...
  if (Array.isArray(graph)) {
    return graph.filter((n): n is JsonLdNode => typeof n === "object" && n !== null);
  }
  return [node];
};
 ...
 */
const missingRequired = (node: JsonLdNode, required: Record<string, string[]>): string[] => {
  const problems: string[] = [];
  for (const type of typesOf(node)) {
    for (const property of required[type] ?? []) {
      if (node[property] === undefined || node[property] === null) {
        problems.push(`${type} is missing the required property "${property}".`);
      }
    }
  }
  return problems;
...
export default defineTool({
  ...
  async execute({ jsonld }, ctx) {
    ...
    try {
      const raw = await ctx.getSkill("schema").file(REQUIRED_PROPERTIES_FILE).text();
    ...
    if (nodes.length === 0) {
      errors.push("No JSON-LD entity found. Expected an object, an array, or an @graph.");
    }
    const isPlainObject = typeof doc === "object" && doc !== null && !Array.isArray(doc);
  ...
  outputSchema: z.object({
    errors: z.array(z.string()).describe("Problems that will stop this working. Fix all of them."),
    types: z
      .array(z.string())
      .describe("Every @type found, so you can confirm the block says what you meant."),
    valid: z
      .boolean()
      .describe(
