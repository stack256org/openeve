 ...
 */
export const buildTrackedLinkTool = (surfaces: readonly [string, ...string[]]) =>
  defineTool({
    description:
      ...
      return {
        ...
        links: links.map(({ url, content }) => {
          ...
          if (normalizedContent) {
            trackingParams.utm_content = normalizedContent;
          }
          const tracked = addTrackingParams(url, trackingParams);
          return tracked
            ? { original: url, tracked }
            : {
                error: "Not an absolute http or https URL, so it was left alone.",
                original: url,
              };
        }),
        medium,
    ...
    inputSchema: z.object({
      ...
      links: z
        ...
        .describe("Every link in this campaign. Pass them together rather than one call per link."),
      surface: z
        .enum(surfaces)
        .describe(
    ...
    outputSchema: z.object({
      campaign: z.string().describe("The normalized utm_campaign that was applied."),
        ...
        .array(
          z.object({
            error: z
              .string()
              .optional()
              .describe("Why this link was left untagged. Absent when it worked."),
            original: z.string(),
            tracked: z
              .string()
              .optional()
