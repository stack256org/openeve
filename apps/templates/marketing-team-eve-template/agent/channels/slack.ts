...
function slackSessionAuth(message: SlackMessage, onBehalfOfUserId?: string) {
  ...
  if (message.teamId) {
    attributes.team_id = message.teamId;
  }
  return {
    attributes,
    ...
    principalType: message.author?.isBot && !onBehalfOfUserId ? "service" : "user",
  };
}
 ...
 */
function firstHumanMention(text: string, botUserId: string): string | undefined {
...
async function handleInbound(ctx: SlackContext, message: SlackMessage) {
  ...
  return {
    auth: slackSessionAuth(message, await resolveOnBehalfOfUserId(ctx, message)),
  };
}
 ...
 */
async function isSoleThreadParticipant(ctx: SlackContext, message: SlackMessage): Promise<boolean> {
 ...
 * ignored.
 */
function suggestedPromptsTarget(event: SlackEvent): SuggestedPromptsTarget | undefined {
  if (event.type === "assistant_thread_started") {
    ...
    const target: { channelId: string; threadTs?: string } = { channelId };
    if (typeof threadTs === "string" && threadTs.length > 0) {
      target.threadTs = threadTs;
    }
    return target;
  }
...
async function setSuggestedPrompts(
  ...
  try {
    ...
    if (target.threadTs) {
      params.thread_ts = target.threadTs;
    }
    const response = await ctx.slack.request("assistant.threads.setSuggestedPrompts", params);
    if (response.ok !== true) {
