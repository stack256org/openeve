...
agent/
  ...
  sandbox.ts                        # Docker sandbox backend for the lead
  channels/
    eve.ts                          # inbound route for the TUI and your own front end
    slack.ts                        # inbound route for Slack, SLACK_BOT_TOKEN + SLACK_SIGNING_SECRET
  tools/
  ...
  lib/                              # shared typed helpers; imported as #lib/<domain>/<file>.js
    assets/{config,tools}.ts        # data directory, key validation, reserved-prefix guards, 5 asset tool factories
    models/config.ts                # every agent's model, in one place
...
| ------------------------ | ---------------------------------------------------------------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| eve channel              | `agent/channels/eve.ts`                                          | channel        | Inbound route for the dev TUI and your own front end. Chains a `localDev()` shim that upgrades the local principal to a user with HTTP Basic, which is only registered when `EVE_API_PASSWORD` is set so an unset variable leaves the route closed.                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Slack channel            | `agent/channels/slack.ts`                                        | channel        | Inbound route for Slack. Answers mentions and DMs, and auto-replies to un-mentioned messages only in a subscribed thread whose original requester is still the sole human participant. The bot token and the signing secret come from the environment.                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| Shared state tools       | `agent/tools/*.ts` + `lib/brand-context`, `lib/user-preferences` | tools          | Read and write the team-wide brand context and the per-user preference document on disk.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| Resend connection        | `.../email/connections/resend.ts`                                | connection     | Remote MCP on a `RESEND_API_KEY` bearer token, the same shape as Notion. The only connection that narrows discovery: `tools.allow` cuts around 85 published tools to the 47 that make up the campaign, list, diagnostic, and read-only-domain surface, leaving out API keys, webhooks, and domain writes entirely. Sends (`send-broadcast`, `send-email`, `send-batch-emails`) and destructive calls always pause for approval, scheduled or not, since Resend splits composing from committing into separate tools. The key is account-scoped, so Resend records the send against the key rather than the person; the approval prompt is what names who agreed to it. |
...
| Asset tools              | `lib/assets/tools.ts`, wired per agent                           | tools          | Write, list, inspect, read, and delete files under `EVE_DATA_DIR/assets`, addressed by a validated key. Deletes pause for approval; the reserved brand-context and preference prefixes are refused.                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| Handoff artifacts        | `lib/artifacts/`                                                 | tool factories | `save_artifact` writes a Markdown document under the reserved `artifacts/` prefix and returns an id; `read_artifact` reads the file back. All five specialists hold both. The lead holds only the reader, so a long document can be relayed between specialists by id without passing through the lead's context. This is also the main channel from the content marketer to the email agent.                                                                                                                                                                                                                                                                          |
...
you
 └─ eve channel (resolve principal)
     └─ lead
         ├─ get_brand_context / get_user_preferences        (disk read)
         ├─ save_brand_context / save_user_preferences      (disk write, ungated)
         └─ one specialist, briefed in full
             ├─ get_brand_context                           (disk read, again: fresh context)
             ...
             ├─ save_artifact -> id                          (long output, reserved prefix)
...
| ---------------- | ------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| Anthropic        | model access for every agent                                        | `@ai-sdk/anthropic` called directly with `ANTHROPIC_API_KEY`; every agent's model lives in `lib/models/config.ts`                 |
| Local filesystem | brand context, per-user preferences, artifacts, assets              | `node:fs/promises` under `EVE_DATA_DIR`, via `lib/assets/config.ts`                                                               |
| Notion           | briefs, calendars, and the finished long-form pieces                | remote MCP at `mcp.notion.com` with a workspace integration token (`NOTION_API_KEY`)                                              |
| Typefully        | social drafts, scheduling, post and follower analytics              | remote MCP at `mcp.typefully.com`, static workspace API key                                                                       |
| Resend           | email campaigns, templates, contacts and segments, delivery records | remote MCP at `mcp.resend.com` with `RESEND_API_KEY`, discovery narrowed by `tools.allow`                                         |
| Docker           | materializes each skill's `references/` files and runs `bash`       | `defineSandbox({ backend: docker() })`                                                                                            |
...
- **Platform:** any host that runs Node 24 and can reach a Docker daemon. `pnpm build` produces the bundle.
- **Stores:** one directory, `EVE_DATA_DIR` (default `./data`). It must be writable and should outlive the process, so mount a volume rather than relying on a container's filesystem.
- **Slack app:** install it in the workspace and point its Event Subscriptions request URL at `<your-host>/eve/v1/slack`.
- **Environment:** every credential is a variable, listed in `.env.example`: `SLACK_BOT_TOKEN` and `SLACK_SIGNING_SECRET`, `NOTION_API_KEY`, `RESEND_API_KEY`, `TYPEFULLY_API_KEY`, and `ANTHROPIC_API_KEY`. The Notion and Resend tokens are account-scoped rather than per-user, so mint each with the narrowest permission that covers the work. Resend also needs a verified sending domain in the workspace before anything can be sent; the agent reads that state but cannot create it.
- **Runtime:** Node 24.x, ESM, `moduleResolution: "bundler"`.
- **Local development:** copy `.env.example` to `.env.local`, fill it in, then `pnpm dev` for the eve TUI. The sandbox needs a running Docker daemon.
...
- **eve** — the agent framework. Discovers an agent's capabilities from the filesystem and produces a deployable app.
...
- **Asset key** — the identifier for a stored file, a relative path such as `drafts/post.md`. Validated against an anchored pattern in `lib/assets/config.ts` before it becomes a real path.
- **`EVE_DATA_DIR`** — the single directory holding everything durable the team writes, defaulting to `./data`.
