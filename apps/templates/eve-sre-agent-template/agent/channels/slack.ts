 ...
 */
async function respondToInboundMessage(ctx: SlackInboundMessageContext, message: SlackMessage) {
  await ctx.thread.startTyping("Thinking...").catch(console.error);
  return { auth: defaultSlackAuth(message, ctx) };
}

function toChannelWatchMessage(message: SlackMessage): ChannelWatchMessage | null {
  const rawTeamId = message.raw.team;
  const teamId = message.teamId ?? (typeof rawTeamId === "string" ? rawTeamId : undefined);
...
function channelWatchAuthor(author: SlackMessage["author"]): ChannelWatchMessage["author"] {
  if (!author) {
    return { kind: "missing" };
  }
  if (author.isMe) {
 ...
 */
const investigationMessageCompleted: NonNullable<SlackChannelEvents["message.completed"]> = async (
  data,
  ctx,
) => {
  if (data.finishReason === "tool-calls") {
    ctx.state.pendingToolCallMessage = data.message ? firstNonEmptyLine(data.message) : null;
    return;
  }
  ...
  try {
    if (evidence.length > 0) {
      await ctx.thread.post(buildInvestigationResultMessage(data.message, evidence));
    } else {
...
export default slackChannel({
  ...
  async onMessage(ctx, message) {
    ...
    const admission = await admitChannelWatchMessage(watched, getChannelWatchStore());
