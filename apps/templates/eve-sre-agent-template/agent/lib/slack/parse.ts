...
export function normalizeSlackTimestamp(value: string, label: string): string {
  ...
  throw new Error(`Could not parse a Slack message timestamp from "${value}" for ${label}.`);
}
...
export function compactSlackMessage(raw: unknown): CompactSlackMessage | undefined {
