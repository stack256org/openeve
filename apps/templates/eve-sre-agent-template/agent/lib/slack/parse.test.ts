...
describe("normalizeSlackChannelId", () => {
  it("accepts channel mentions, permalinks, and bare IDs", () => {
    assert.equal(normalizeSlackChannelId("<#C12345678|incidents>"), "C12345678");
    assert.equal(
      normalizeSlackChannelId("https://example.slack.com/archives/C87654321/p1770000000000000"),
      "C87654321",
    );
  ...
  it("rejects values that carry no channel ID", () => {
    assert.throws(() => normalizeSlackChannelId("not a channel"), CHANNEL_ID_ERROR);
  });
});
...
describe("normalizeSlackTimestamp", () => {
  it("accepts permalink timestamps and raw timestamps", () => {
    ...
    assert.equal(normalizeSlackTimestamp("1770000000.123456", "messageTs"), "1770000000.123456");
  });

  it("rejects unparseable values", () => {
    assert.throws(() => normalizeSlackTimestamp("yesterday", "messageTs"), TIMESTAMP_ERROR);
  });
});
