...
import { type CustomSkillStore, getCustomSkillStore, type StoredSkill } from "#lib/skills/store.ts";
...
export async function resolveCustomSkills(
  ...
  const globalSkills = globalResult.status === "fulfilled" ? globalResult.value : [];
