  ...
  const resolved = await resolveCustomSkills(
    { principalId: "eve:app", principalType: "service" },
    {
      list(auth, scope) {
        ...
        return Promise.resolve(auth?.principalType === "user" ? [userSkill] : []);
      },
    },
  );

  assert.deepEqual(Object.keys(resolved ?? {}), ["custom-deploy"]);
  assert.equal(resolved?.["custom-deploy"]?.description, globalSkill.description);
...
test("personal skills shadow global skills for a human Slack user", async () => {
  const resolved = await resolveCustomSkills(
    ...
    {
      list(_auth, scope) {
        return Promise.resolve(scope === "global" ? [globalSkill] : [userSkill]);
      },
    },
  );
