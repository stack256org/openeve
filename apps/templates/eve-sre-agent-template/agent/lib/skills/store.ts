...
export function normalizeSkillName(raw: string): string {
  const normalized = pathSafe(raw.trim()).slice(0, MAX_SKILL_NAME_LENGTH).replace(/-+$/g, "");
  if (!normalized) {
...
function validateSkillContent(input: SaveSkillInput): {
  ...
  if (markdown.includes("\0")) {
    throw new InvalidSkillContentError("Skill instructions cannot contain null characters.");
  }
...
export class CustomSkillStore {
  ...
  async save(
    ...
    await this.objects.write(skillPath(user, scope, name), JSON.stringify(stored));
    return stored;
  }
  ...
  async list(
    ...
    const skills = await Promise.all(pathnames.map((pathname) => this.readPath(pathname)));
