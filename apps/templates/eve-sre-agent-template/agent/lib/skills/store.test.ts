...
class MemoryObjectStore implements BlobObjectStore {
  ...
  list(prefix: string): Promise<string[]> {
    return Promise.resolve(
      [...this.objects.keys()].filter((objectPath) => objectPath.startsWith(prefix)),
    );
  }
...
test("stores global and personal skills independently", async () => {
  ...
  assert.equal((await store.list(null, "global"))[0]?.markdown, "# Global deploy");
  assert.equal((await store.list(alice, "user"))[0]?.markdown, "# Alice deploy");
  assert.deepEqual(
...
test("service callers list global skills without personal skills", async () => {
  ...
  await assert.rejects(store.delete(service, "global", "deploy"), MissingHumanUserError);
});
