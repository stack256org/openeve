...
import { createServiceAuth, resolveSlackUser, type SlackUserAuthLike } from "./auth.ts";

function slackUserAuth(overrides: Partial<SlackUserAuthLike> = {}): SlackUserAuthLike {
  return {
...
describe("resolveSlackUser", () => {
  ...
  it("rejects inconsistent Slack identities", () => {
    ...
    assert.equal(resolveSlackUser(slackUserAuth({ attributes: { team_id: " " } })), null);
    assert.equal(resolveSlackUser(slackUserAuth({ issuer: "slack:T999" })), null);
    ...
    assert.equal(resolveSlackUser(slackUserAuth({ principalId: undefined })), null);
  });
