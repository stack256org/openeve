...
export type InvestigationEvidenceSourceType = (typeof INVESTIGATION_EVIDENCE_SOURCE_TYPES)[number];
...
export function addInvestigationEvidence(
  ...
  const existing = current.items.find(
    (candidate) =>
      candidate.sourceUrl === normalized.sourceUrl && candidate.finding === normalized.finding,
  );
...
export function removeInvestigationEvidence(
  ...
  investigationEvidenceState.update((state) => {
    const { removed: nextRemoved, state: nextState } = removeInvestigationEvidenceFromState(
      state,
      turnId,
      evidenceId,
    );
...
export function getInvestigationEvidence(turnId: string): readonly InvestigationEvidence[] {
...
function evidenceLines(
  ...
  return items.map(
    (item) => `${bullet} ${link(item.sourceLabel, item.sourceUrl)} — ${item.finding}`,
  );
}
