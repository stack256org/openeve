...
import { readWebhookJson, readWebhookSecret, verifyWebhookSecret } from "./request.ts";
...
test("reads a bearer token", () => {
  assert.equal(readWebhookSecret(new Headers({ authorization: "Bearer secret" })), "secret");
});

test("rejects missing, malformed, and mismatched secrets", () => {
  assert.equal(verifyWebhookSecret(new Headers(), "secret"), false);
  assert.equal(
    verifyWebhookSecret(new Headers({ authorization: "Basic secret" }), "secret"),
    false,
  );
  assert.equal(
    verifyWebhookSecret(new Headers({ "x-sre-webhook-secret": "wrong" }), "secret"),
    false,
  );
  assert.equal(
    verifyWebhookSecret(new Headers({ "x-sre-webhook-secret": "secret" }), undefined),
    false,
  );
});
...
test("accepts matching secrets", () => {
  assert.equal(
    verifyWebhookSecret(new Headers({ "x-sre-webhook-secret": "secret" }), "secret"),
    true,
  );
});
