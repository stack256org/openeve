...
function evidence(overrides: Partial<InvestigationEvidence> = {}): InvestigationEvidence {
  return {
...
test("addInvestigationEvidence preserves discovery order and normalizes text", () => {
  ...
  assert.deepEqual(
    ...
    ["Errors rose rapidly.", "A deployment completed immediately before the spike."],
  );
...
test("removeInvestigationEvidenceFromState removes by stable ID without reusing IDs", () => {
  ...
  const removed = removeInvestigationEvidenceFromState(third.state, "turn-1", "evidence-2");
...
test("removeInvestigationEvidenceFromState cannot remove evidence from another turn", () => {
  const firstTurn = addInvestigationEvidence(initialState, "turn-1", evidence());
  const result = removeInvestigationEvidenceFromState(firstTurn.state, "turn-2", "evidence-1");
...
test("appendInvestigationEvidenceToAnswer leaves unrelated unsourced answers unchanged", () => {
  assert.equal(appendInvestigationEvidenceToAnswer("Hello there.", []), "Hello there.");
});

test("appendInvestigationEvidenceToAnswer appends recorded evidence", () => {
  assert.equal(
    appendInvestigationEvidenceToAnswer("Likely root cause: overload.", [evidence()]),
    [
