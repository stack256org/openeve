...
test("returns each structural drop reason in policy order", () => {
  ...
  assert.deepEqual(evaluateChannelWatchMessage({ ...message, subtype: "message_changed" }), {
    kind: "drop",
    reason: "ignored_subtype",
  });
  assert.deepEqual(evaluateChannelWatchMessage({ ...message, author: { kind: "missing" } }), {
    kind: "drop",
    reason: "missing_author",
  });
  assert.deepEqual(evaluateChannelWatchMessage({ ...message, author: { kind: "self" } }), {
    kind: "drop",
    reason: "bot_author",
  });
  assert.deepEqual(evaluateChannelWatchMessage({ ...message, visibleText: "" }), {
    kind: "drop",
    reason: "empty_text",
  });
});
...
test("admits attachment-only Datadog bot messages when watched", async () => {
  ...
  assert.equal(result.kind, "admit");
  if (result.kind === "admit") {
    assert.equal(result.message.visibleText, "High error rate on checkout\np95 is above 2 seconds");
...
test("missing team or channel ids do not read membership", async () => {
  ...
  assert.deepEqual(await admitChannelWatchMessage({ ...message, teamId: "" }, membership), {
    kind: "drop",
    reason: "not_watched",
  });
  assert.deepEqual(await admitChannelWatchMessage({ ...message, channelId: "" }, membership), {
    kind: "drop",
    reason: "not_watched",
  });
  assert.equal(calls, 0);
