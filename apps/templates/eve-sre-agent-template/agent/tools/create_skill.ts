...
export const createSkillInputSchema = z.object({
  ...
  name: z
    ...
    .describe("Short identifier, normalized to lowercase letters, numbers, and hyphens."),
  scope: z
    .enum(["global", "user"])
    .describe("Whether the skill is available to everyone, or just the current user."),
});
  ...
  async execute(input, ctx) {
    ...
    if (!resolveSlackUser(current)) {
      console.error("[sre/custom-skills] create_skill authorization denied", {
        ...
        teamId: typeof current?.attributes.team_id === "string" ? current.attributes.team_id : null,
        userId: typeof current?.attributes.user_id === "string" ? current.attributes.user_id : null,
      });
      return {
    ...
    try {
      const skill = await getCustomSkillStore().save(ctx.session.auth.current, input.scope, {
      ...
      return {
        error: "Failed to save the skill. The skill store may not be configured.",
        success: false,
      };
    }
  },
