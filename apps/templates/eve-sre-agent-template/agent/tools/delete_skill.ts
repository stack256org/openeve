...
import { getCustomSkillStore, MAX_SKILL_NAME_LENGTH, SkillStoreError } from "#lib/skills/store.ts";

export const deleteSkillInputSchema = z.object({
  name: z.string().min(1).max(MAX_SKILL_NAME_LENGTH).describe("The name of the skill to delete."),
  scope: z.enum(["global", "user"]).describe("The scope of the skill to delete."),
});
...
export default defineTool({
  ...
  async execute(input, ctx) {
    ...
    try {
      await getCustomSkillStore().delete(ctx.session.auth.current, input.scope, input.name);
      return {
      ...
      return {
        error: "Failed to delete the skill. The skill store may not be configured.",
        success: false,
      };
    }
  },
