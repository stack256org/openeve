...
const watchSlackChannelInputSchema = z.object({
  channel: z
    .string()
    .min(1)
    .describe("Slack channel ID, channel mention, or Slack permalink to watch."),
});
  ...
  async execute(input, ctx) {
    ...
    try {
      ...
      if (channel.is_im || channel.is_mpim) {
        return {
          error: "Channel watch supports only public or private Slack channels.",
          success: false,
        };
      }
      ...
      return {
        error: error instanceof Error ? error.message : "Failed to watch the Slack channel.",
        success: false,
      };
    }
  },
