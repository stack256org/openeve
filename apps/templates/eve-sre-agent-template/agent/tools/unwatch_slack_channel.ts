...
const unwatchSlackChannelInputSchema = z.object({
  channel: z
    .string()
    .min(1)
    .describe("Slack channel ID, channel mention, or Slack permalink to stop watching."),
});

export default defineTool({
  approval: always(),
  description: "Stop watching a Slack channel. New messages will no longer start investigations.",
  async execute(input, ctx) {
    ...
    try {
      ...
      console.error("[sre/channel-watch] failed to unwatch Slack channel", error);
      return {
        error: error instanceof Error ? error.message : "Failed to unwatch the Slack channel.",
        success: false,
      };
    }
  },
