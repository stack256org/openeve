...
When the user asks you to set up the project, follow [`docs/setup-for-agents.md`](./docs/setup-for-agents.md). You are already at the repo root.
...
- The user sets secrets in their host's environment. Never read `.env` files.
- Never commit secrets. `.env*` is gitignored. `.env.example` is the exception.
- Every integration credential comes from env (`SLACK_BOT_TOKEN`, `GITHUB_TOKEN`, `DD_API_KEY`, `DD_APPLICATION_KEY`). Tokens are resolved per call and are not exposed to the model.
