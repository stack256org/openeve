...
This is separate from Sanity and Notion: the asset store is for files, like exporting a finished piece as Markdown, saving an image, or keeping anything that should outlast the conversation. Each asset has a key, a relative path with an extension, and that key is how anyone reads it back.
